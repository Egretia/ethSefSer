...
Agent.prototype = {
    approve : function (tokenAddr, transactionObject, toAddr, amount) {
        var web3 = this.web3 ;

        return new Promise(function (resole, reject) {
            let calcContract = web3.eth.contract(Erc20Abi.abi);
            let contractInstance = calcContract.at(tokenAddr);
            contractInstance.approve.sendTransaction(toAddr, amount, transactionObject,
                function (err, txhash) {
                    let recvTxHash = false ;
                    let recvError = "" ;
                    
                    if (err) {
                        LogUtil.error("addEgtCollect send ERC20 Token failed ", err)
                        recvTxHash = false ;
                        recvError = err.message ;
                    } else {
                        LogUtil.info("addEgtCollect send ERC20 token done ", txhash)
                        recvTxHash = txhash ;
                        recvError = false ;
                    }
...
EthUtil.newAddresses = (ks, password, num) => {
    return new Promise(function (resole, reject) {
        ks.keyFromPassword(password, function (err, pwDerivedKey) {
            if (err) {
                LogUtil.error("New address failed : "+err.message);
