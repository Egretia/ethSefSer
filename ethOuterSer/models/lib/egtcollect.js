...
Collect.setErrByApplyId = async (applyId, errmsg) => {
    let data = {
    ...
    let sql = "update eth_collect set "+fields.join(",")+" where apply_id="+applyId ;
...
Collect.getByApplyId = async (applyid) =>{
    let result = await Query.sql_select("select * from eth_collect where apply_id="+applyid+" limit 1")
...
// 给需要聚合的地址充eth gas费用
Collect.recordEthGas = async (applyid,eth,txhash)=>{
